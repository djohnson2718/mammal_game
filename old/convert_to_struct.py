...
order_to_members = dict()
for member, order in names_to_order.items():
    if order in order_to_members:
        order_to_members[order].append(member)
    else:
        order_to_members[order] = [member]

for order, members in order_to_members.items():
    print( """
    new Order('{0}',
    'desc',
    'pic_name',
